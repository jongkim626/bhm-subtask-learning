...
###   4.  Varying Intercept Mixed Model
###   5.  Varying Intercept and Varying Slopes Mixed Model
###   6.  Consideration of Correlation 
#
#Set up the environment for Bayesian computations
###   5.  The Models
###############################################################################
...
setwd("~/rdata/bhm-subtask-learning")
...
## load data 
herbal.data <- read.csv("~/rdata/bhm-subtask-learning/herbal-data.csv", header = T)
attach(herbal.data)
names(herbal.data)
head(herbal.data)
...
subtask.learning.k <- read.table("~/rdata/bhm-subtask-learning/sl-data.txt", header=T)
...
# some of the observations, missing values for certain covariates. 
...
varIntFit <- stan(file = "bhm-subtask-VarInt.stan",
                 data = stanDat,
                 iter = 2000,
                 chains = 4)
...
ranIntSlpNoCorFit <- stan(file = "bhm-subtask-VarIntVarSlp.stan", 
                          data = stanDat,
                          iter = 2000, chains =4)
...
## fit model
VarIntVarSlpCorFit <- stan(file = "bhm-subtask-VarIntVarSlpCorFit.stan", 
                          data = stanDat,
                          iter = 2000, chains =4)
...
####   End of the code for Bayesian Hierarchical Model and Inference 
...
###
###	  linear mixed effects model (Kim & Ritter, 2016)
###  
...
################################################################################
###
###	  Subtask Learning Analytics-- Bayesian Approach
###
################################################################################

##
##  Set up the environments-- packages and library
##
